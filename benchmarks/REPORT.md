# Benchmark Report: Ultra-Metis vs Original Metis

**Date**: 2026-03-17
**Environment**: macOS Darwin 24.4.0
**Ultra-Metis**: Release binary (Rust CLI)
**Original Metis**: MCP plugin (TypeScript, JSON-RPC over stdio)

## Executive Summary

Ultra-metis is **~200x faster** than original metis for individual operations due to the elimination of MCP protocol overhead. However, original metis has **better validation**, **richer templates**, and **more robust error handling**. The speed advantage of ultra-metis is dramatic and real, but correctness gaps need to be addressed before it can replace the original.

## Methodology

Both systems were given identical tasks using fresh project directories (`/tmp/bench-ultra-metis` and `/tmp/bench-metis-original`). Ultra-metis was invoked directly as a CLI binary; original metis was invoked via MCP tool calls from within a Claude Code session. Timestamps were captured using Python `time.time()` with millisecond precision.
...
| Metric | Ultra-Metis | Original Metis | Winner |
|--------|-------------|----------------|--------|
| Time | **38ms** | 7,739ms | Ultra-Metis (204x) |
| Output size | 75 bytes | 393 bytes | Ultra-Metis (leaner) |
| Pass/Fail | PASS | PASS | Tie |

**Notes**: Ultra-metis creates a `.ultra-metis/` directory with config. Original metis creates `.metis/` with SQLite DB, vision template, and config. Both succeed, but metis does more setup work.

### Scenario 2: Planning Workflow (Vision + Initiative + 3 Tasks)

| Operation | Ultra-Metis | Original Metis |
|-----------|-------------|----------------|
...
| Metric | Ultra-Metis | Original Metis | Winner |
|--------|-------------|----------------|--------|
| Total time | **181ms** | 33,568ms | Ultra-Metis (185x) |
| Total output | 322 bytes | 1,520 bytes | Ultra-Metis (leaner) |
...
| Transition | Ultra-Metis | Original Metis |
|-----------|-------------|----------------|
...
| Metric | Ultra-Metis | Original Metis | Winner |
|--------|-------------|----------------|--------|
| Total time | **195ms** | 49,113ms | Ultra-Metis (252x) |
| Avg per transition | **39ms** | 9,823ms | Ultra-Metis |
| Consistency | 35-42ms range | 7-13s range | Ultra-Metis |
...
| Metric | Ultra-Metis | Original Metis | Winner |
|--------|-------------|----------------|--------|
| Avg search time | **38ms** | 10,068ms | Ultra-Metis (265x) |
| Search precision | Title-only match | Full-text (title+body) | Context-dependent |
| Search "API" accuracy | 2 results (correct) | 9 results (false positives) | Ultra-Metis |
...
| Error Case | Ultra-Metis | Original Metis |
|-----------|-------------|----------------|
...
| Avg error time | 41ms | 9,287ms | Ultra-Metis |

**Critical findings for Ultra-Metis**:
...
| Metric | Ultra-Metis | Original Metis | Winner |
|--------|-------------|----------------|--------|
| Read time | **39ms** | 10,218ms | Ultra-Metis (262x) |
...
| Scenario | Ultra-Metis | Original Metis | Speedup |
|----------|-------------|----------------|---------|
...
| Dimension | Ultra-Metis | Original Metis | Winner |
|-----------|-------------|----------------|--------|
| Speed | 5/5 | 1/5 | Ultra-Metis |
...
| Search precision | 4/5 | 3/5 | Ultra-Metis |
| Output conciseness | 5/5 | 3/5 | Ultra-Metis |

### Token Usage Implications

For AI agent workflows, output size directly impacts token consumption:

| Workflow | Ultra-Metis Tokens (est.) | Original Metis Tokens (est.) | Savings |
|----------|---------------------------|------------------------------|---------|
| Init | ~20 | ~100 | 80% |
...
### Ultra-Metis Strengths (keep/enhance)
...
### Ultra-Metis Gaps (fix before shipping)
...
However, the validation and error handling gaps are real. An agent that can create orphan documents or silently fail on invalid transitions will produce corrupted project state. These must be fixed before ultra-metis can reliably replace original metis in production workflows.

**Bottom line**: Ultra-metis is faster and leaner. Original metis is more correct and feature-rich. The path forward is clear: fix the validation bugs, enrich the templates, and ultra-metis will be strictly better.
