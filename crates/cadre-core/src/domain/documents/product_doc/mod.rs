...
/// A ProductDoc defines the top-level product intent, scope, and success criteria.
/// Replaces Vision as the top of the planning hierarchy.
...
impl ProductDoc {
    ...
    pub fn new_with_template(
        ...
        tera.add_raw_template("product_doc_content", template_content)
            .map_err(|e| {
        ...
        let rendered_content = tera.render("product_doc_content", &context).map_err(|e| {
    ...
    pub fn from_content(raw_content: &str) -> Result<Self, DocumentValidationError> {
        ...
        if level != "product_doc" {
            return Err(DocumentValidationError::InvalidContent(format!(
                "Expected level 'product_doc', found '{level}'"
            )));
        }
...
impl Document for ProductDoc {
    ...
    fn can_transition_to(&self, phase: Phase) -> bool {
        if let Ok(current_phase) = self.phase() {
            DocumentType::ProductDoc.can_transition(current_phase, phase)
        } else {
            false
        }
    }
    ...
    fn validate(&self) -> Result<(), DocumentValidationError> {
        if self.title().trim().is_empty() {
            return Err(DocumentValidationError::InvalidTitle(
                "ProductDoc title cannot be empty".to_string(),
            ));
        }
        if self.parent_id().is_some() {
            return Err(DocumentValidationError::InvalidParent(
                "ProductDocs cannot have parents".to_string(),
            ));
        }
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn test_product_doc_creation() {
        let pd = ProductDoc::new(
            "My Product".to_string(),
            vec![Tag::Phase(Phase::Draft)],
            false,
            "TEST-PD-0001".to_string(),
        )
        .unwrap();

        assert_eq!(pd.title(), "My Product");
        assert_eq!(pd.document_type(), DocumentType::ProductDoc);
        assert_eq!(pd.phase().unwrap(), Phase::Draft);
        assert!(pd.validate().is_ok());
        assert!(pd.parent_id().is_none());
    }

    #[tokio::test]
    async fn test_product_doc_roundtrip() {
        let pd = ProductDoc::new(
            "Test Product".to_string(),
            vec![
                Tag::Label("product_doc".to_string()),
                Tag::Phase(Phase::Draft),
            ],
            false,
            "TEST-PD-0002".to_string(),
        )
        .unwrap();

        let temp_dir = tempdir().unwrap();
        let file_path = temp_dir.path().join("test-product-doc.md");

        pd.to_file(&file_path).await.unwrap();
        let loaded = ProductDoc::from_file(&file_path).await.unwrap();

        assert_eq!(loaded.title(), pd.title());
        assert_eq!(loaded.phase().unwrap(), pd.phase().unwrap());
        assert_eq!(loaded.tags().len(), pd.tags().len());
    }

    #[test]
    fn test_product_doc_transitions() {
        let mut pd = ProductDoc::new(
            "Test Product".to_string(),
            vec![Tag::Phase(Phase::Draft)],
            false,
            "TEST-PD-0003".to_string(),
        )
        .unwrap();

        assert!(pd.can_transition_to(Phase::Review));
        assert!(!pd.can_transition_to(Phase::Published));

        let new_phase = pd.transition_phase(None).unwrap();
