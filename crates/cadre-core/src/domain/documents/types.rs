...
/// Document type enumeration — Cadre engineering-oriented hierarchy
///
...
impl DocumentType {
    ...
    /// Returns true if this is a new Cadre type (not legacy)
...
mod tests {
    ...
    #[test]
    fn test_is_cadre_type() {
        assert!(DocumentType::ProductDoc.is_cadre_type());
        assert!(DocumentType::Epic.is_cadre_type());
        assert!(DocumentType::Story.is_cadre_type());
        assert!(DocumentType::DesignContext.is_cadre_type());
        assert!(!DocumentType::Vision.is_cadre_type());
        assert!(!DocumentType::Task.is_cadre_type());
    }
