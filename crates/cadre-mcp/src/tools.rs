...
use cadre_core::domain::rules::query::{RuleQuery, RuleQueryEngine, scope_hierarchy};
use cadre_store::{CodeIndexer, DocumentStore};
...
pub fn get_tool_definitions() -> Vec<ToolDefinition> {
    vec![
        ToolDefinition {
            ...
            description: "Initialize a new Cadre project directory".to_string(),
...
fn tool_index_code(args: &Value) -> Result<String, String> {
    ...
    if let Some(q) = query {
        ...
        let index: cadre_store::CodeIndex = serde_json::from_str(&index_content)
...
fn tool_check_architecture_conformance(args: &Value) -> Result<String, String> {
    ...
    if doc.document_type() != cadre_core::DocumentType::ReferenceArchitecture {
        return Err(format!(
...
fn tool_query_architecture_catalog(args: &Value) -> Result<String, String> {
    ...
    let mut query = cadre_core::CatalogQuery::new();
...
fn tool_evaluate_brownfield(args: &Value) -> Result<String, String> {
    ...
    let outcome_desc = match &result.outcome {
        cadre_core::EvaluationOutcome::CatalogMatch { catalog_entry_id, .. } =>
        ...
        cadre_core::EvaluationOutcome::RecommendCatalogPattern { recommended_entry_id, .. } =>
            format!("Recommend Catalog Pattern ({})", recommended_entry_id),
        cadre_core::EvaluationOutcome::RecordAsIs { .. } =>
...
fn tool_query_rules(args: &Value) -> Result<String, String> {
    ...
    if let Some(s) = scope_str {
        let scope = s.parse().map_err(|e: cadre_core::DocumentValidationError| e.to_string())?;
        query = query.with_scope(scope);
    }
    if let Some(p) = protection_str {
        let level = p.parse().map_err(|e: cadre_core::DocumentValidationError| e.to_string())?;
...
fn tool_get_applicable_rules(args: &Value) -> Result<String, String> {
    ...
    let hierarchy: Vec<&str> = scope_hierarchy()
        .iter()
        .take_while(|s| {
            cadre_core::domain::rules::query::scope_rank(**s)
                <= cadre_core::domain::rules::query::scope_rank(target_scope)
        })
        .map(|s| match s {
