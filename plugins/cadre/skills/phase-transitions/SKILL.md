...
This skill guides moving Cadre documents through their lifecycle phases.
...
**To complete a task**, call `mcp__cadre__transition_phase` twice:
1. `transition_phase(short_code)` → todo to active
2. `transition_phase(short_code)` → active to completed

**To publish a ProductDoc**, call `mcp__cadre__transition_phase` twice:
...
```
mcp__cadre__transition_phase(short_code="PROJ-E-0001")
```
Moves to next valid phase. Validates exit criteria.

**Explicit phase (for blocked state):**
```
mcp__cadre__transition_phase(short_code="PROJ-S-0042", phase="blocked")
```
Use explicit phase only for moving to/from blocked state (stories and tasks only).

**Force (use sparingly):**
```
mcp__cadre__transition_phase(short_code="PROJ-E-0001", force=true)
```
...
1. Transition to blocked: `mcp__cadre__transition_phase(short_code, phase="blocked")`
...
Use `mcp__cadre__edit_document` to update progress.
