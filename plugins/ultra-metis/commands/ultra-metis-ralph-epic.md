---
description: "Execute all stories under an Ultra-Metis epic sequentially"
argument-hint: "SHORT_CODE [--max-iterations N]"
allowed-tools: ["Bash(${CLAUDE_PLUGIN_ROOT}/scripts/setup-ultra-metis-ralph.sh:*)", "mcp__ultra-metis__read_document", "mcp__ultra-metis__list_documents"]
hide-from-slash-command-tool: "true"
---

# Ultra-Metis Ralph Epic - Multi-Story Execution

## Step 1: Verify Epic and List Stories

Parse the SHORT_CODE from: `$ARGUMENTS`

1. Use `mcp__ultra-metis__read_document` to verify the epic exists
2. Use `mcp__ultra-metis__list_documents` to find all stories under this epic
3. Filter to stories in `todo` or `discovery` phase (not yet started)

**If no stories found**: Tell the user to run `/ultra-metis-decompose` first. Stop here.
...
```bash
"${CLAUDE_PLUGIN_ROOT}/scripts/setup-ultra-metis-ralph.sh" <STORY_SHORT_CODE>
```
