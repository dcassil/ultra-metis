    ...
    cat << 'ENDJSON'
{
    "hookSpecificOutput": {
        ...
        "additionalContext": "WARNING: This is an Ultra-Metis project (`.metis` directory found) but the `ultra-metis` command is not installed or not in PATH. Run `make install` from the ultra-metis repo root."
    }
}
ENDJSON
    exit 0
...
This is an **Ultra-Metis project** (detected \`.metis\` directory).

## Planning Hierarchy
ProductDoc -> Epic -> Story -> Task

## CRITICAL: Work Tracking Rules
- **Do NOT use TodoWrite** for tracking work in this project. Ultra-Metis documents ARE your work tracking system.
- **ALWAYS update active documents** with progress as you work - they serve as persistent memory across sessions.
- Before starting work, check for active work with \`mcp__ultra-metis__list_documents\`.
...
## MCP Tools (Preferred)
Use these MCP tools for all Ultra-Metis operations:
- \`mcp__ultra-metis__list_documents\` - List all documents with short codes and phases
- \`mcp__ultra-metis__read_document\` - Read a document by short code (e.g., PROJ-E-0001)
- \`mcp__ultra-metis__edit_document\` - Update document content (search and replace)
- \`mcp__ultra-metis__transition_phase\` - Move documents through phases
- \`mcp__ultra-metis__create_document\` - Create product_doc, epic, story, task, design_context, or ADR
- \`mcp__ultra-metis__reassign_parent\` - Move stories/tasks between parents
- \`mcp__ultra-metis__search_documents\` - Full-text search across documents
- \`mcp__ultra-metis__archive_document\` - Archive document and children
- \`mcp__ultra-metis__index_code\` - Index source code symbols
...
1. \`mcp__ultra-metis__create_document\` - Creates document with template
2. \`mcp__ultra-metis__read_document\` - Read the template structure
3. \`mcp__ultra-metis__edit_document\` - Replace ALL placeholders with real content
...
1. \`mcp__ultra-metis__read_document\` - Read the document to understand requirements
2. \`mcp__ultra-metis__transition_phase\` - Transition to "active"
3. Work on it, updating the document with progress regularly
4. \`mcp__ultra-metis__transition_phase\` - Transition to "completed" when done
EOF
