#!/bin/bash
# Ultra-Metis Ralph Setup Script
...
while [[ $# -gt 0 ]]; do
  case $1 in
    -h|--help)
      cat << 'HELP_EOF'
Ultra-Metis Ralph - Execute a story or task with Ralph loop

USAGE:
  /ultra-metis-ralph <SHORT_CODE> [OPTIONS]

ARGUMENTS:
  SHORT_CODE    Ultra-Metis short code (e.g., PROJ-S-0001 or PROJ-T-0001)
...
if [[ -z "$SHORT_CODE" ]]; then
  ...
  echo "Usage: /ultra-metis-ralph <SHORT_CODE>" >&2
  exit 1
fi
...
if ! [[ "$SHORT_CODE" =~ ^[A-Z]+-[ST]-[0-9]+$ ]]; then
  ...
  echo "Use /ultra-metis-decompose for epics (PREFIX-E-NNNN)" >&2
  exit 1
fi
...
PROMPT="Execute Ultra-Metis document: $SHORT_CODE
...
1. Read the document using mcp__ultra-metis__read_document(project_path=\"$METIS_PATH\", short_code=\"$SHORT_CODE\") to get current state
2. Transition to active using mcp__ultra-metis__transition_phase(project_path=\"$METIS_PATH\", short_code=\"$SHORT_CODE\")
...
5. Log progress to the document using mcp__ultra-metis__edit_document after each significant step
...
cat <<EOF
Ultra-Metis Ralph activated for: $SHORT_CODE
