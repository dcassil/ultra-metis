#!/bin/bash
# Ultra-Metis Decompose Setup Script
...
while [[ $# -gt 0 ]]; do
  case $1 in
    -h|--help)
      cat << 'HELP_EOF'
Ultra-Metis Decompose - Break an epic into typed stories

USAGE:
  /ultra-metis-decompose <SHORT_CODE> [OPTIONS]

ARGUMENTS:
  SHORT_CODE    Ultra-Metis epic short code (e.g., PROJ-E-0001)
...
if [[ -z "$SHORT_CODE" ]]; then
  ...
  echo "Usage: /ultra-metis-decompose <SHORT_CODE>" >&2
  exit 1
fi
...
if ! [[ "$SHORT_CODE" =~ ^[A-Z]+-E-[0-9]+$ ]]; then
  ...
  echo "Use /ultra-metis-ralph for stories (PREFIX-S-NNNN) or tasks (PREFIX-T-NNNN)" >&2
  exit 1
fi
...
DOC_RAW=$(ultra-metis read "$SHORT_CODE" -p "$PROJECT_PATH" 2>&1) || {
...
PROMPT="Decompose Ultra-Metis epic: $SHORT_CODE
...
2. Invoke skill: ultra-metis:decomposition - Follow decomposition methodology for story breakdown

## Decomposition Instructions

1. Read the epic using mcp__ultra-metis__read_document(project_path=\"$METIS_PATH\", short_code=\"$SHORT_CODE\")
2. Transition to decompose using mcp__ultra-metis__transition_phase(project_path=\"$METIS_PATH\", short_code=\"$SHORT_CODE\")
...
6. After human approval, create each story using mcp__ultra-metis__create_document:
   - type=\"story\"
...
7. IMMEDIATELY populate each story with real content using mcp__ultra-metis__edit_document:
   - Clear description of what to build/fix/change
   - Acceptance criteria
   - Technical notes and constraints
   - A document with template placeholders is INCOMPLETE
8. Log progress to the epic document using mcp__ultra-metis__edit_document
...
cat <<EOF
Ultra-Metis Decompose activated for: $SHORT_CODE
