...
level: task
title: "Close MCP Template Quality Gap: Cadre 44% vs Original Metis 67%"
...
# Close MCP Template Quality Gap: Cadre 44% vs Original Metis 67%
...
SMET-I-0037 rewrote templates in `cadre-core` and claimed 5/5 quality. However, the new MCP apples-to-apples benchmark (`benchmarks/MCP_COMPARISON.md`) shows the templates served via the MCP server still underperform:

- **Cadre: 44% completeness, 11.3 placeholders/doc**
- **Original metis: 67% completeness, 9.7 placeholders/doc**

The JSON Transformer module scored 0% completeness with cadre vs 67% with original metis. This means either:
...
- [ ] Cadre avg placeholders/doc <= original metis (currently 11.3 vs 9.7)
...
### Key Files
- `crates/cadre-core/src/domain/documents/` — core templates
- `crates/cadre-store/` — file persistence layer (may have its own template logic)
- `crates/cadre-mcp/src/tools.rs` — MCP tool handlers
...
- Cadre: 4286 chars — richer than original (HTML comments, detailed placeholder descriptions)
...
| Metric | Cadre | Original Metis |
|--------|:-----------:|:--------------:|
| Avg completeness | 75% | 83% |
| Avg placeholders/doc | 11.0 | 2.3 |

Scoring fix alone raised cadre from 44% → 75%. But placeholder gap remained: cadre template uses verbose `{long description text}` patterns while original metis uses short `{placeholder}` patterns. Haiku doesn't fully replace the verbose ones.

### Template Fix

Simplified cadre initiative template (`content.md`):
...
**Cadre now leads on completeness** (85% vs 55%). Original metis had a flaky CSV fill (0%). Placeholder count is higher for cadre but this is partly an artifact — Haiku generates JSON/code blocks with `{...}` that the regex counts as placeholders.
