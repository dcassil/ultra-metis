...
level: task
title: "Verify cargo build and tests pass, then remove old super-metis/ and metis/ directories"
...
# Verify cargo build and tests pass, then remove old super-metis/ and metis/ directories
...
Run `cargo build` and `cargo test` from the repo root to confirm the workspace compiles and all tests pass after the restructure. Then delete the now-empty `super-metis/` directory and the retired `metis/` reference directory. Commit the completed restructure.
...
- [ ] `super-metis/` directory does not exist (or is empty and deleted)
...
3. Confirm `super-metis/` is empty (crates and Cargo files were moved in prior tasks), then `rm -rf super-metis/`
...
### 2026-03-17
Pre-deletion audit:
- super-metis/crates/ — empty (all 4 crates moved in T-0092) ✓
- super-metis/Cargo.toml and Cargo.lock — removed in T-0093 ✓
- super-metis/tests/e2e_test.sh → moved to tests/ (path resolution works: SCRIPT_DIR resolves to repo root) ✓
- super-metis/benchmarks/ → moved to benchmarks/ at repo root ✓
- super-metis/plugin.json → updated path from ${pluginDir}/../Cargo.toml to ${pluginDir}/Cargo.toml, moved to repo root ✓
- super-metis/.mcp.json → moved to repo root in T-0094 ✓

Final verification:
- cargo build: 0 errors (23 pre-existing warnings only) ✓
- cargo test: 789 passed, 0 failed ✓
- rm -rf super-metis/ and metis/ ✓
