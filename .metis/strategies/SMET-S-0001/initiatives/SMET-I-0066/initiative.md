---
id: ultra-metis-plugin-lifecycle-hooks
level: initiative
title: "Ultra-Metis Plugin: Lifecycle Hooks"
...
initiative_id: ultra-metis-plugin-lifecycle-hooks
---

# Ultra-Metis Plugin: Lifecycle Hooks
...
Ultra-Metis has no lifecycle hooks. When a Claude Code session starts in a project with `.metis`, there's no automatic context loading, no active work summary, and no guidance on what to do next.
...
**Goals:**
- Create a `SessionStart` hook that detects ultra-metis projects and surfaces active work
- Hook should list active initiatives with current phase
- Hook should list actionable tasks (todo, active, blocked)
- Hook should remind about available ultra-metis skills and commands
...
- Hooks that require ultra-metis binary execution (use prompt-based hooks, not shell scripts)
...
2. Call `mcp__ultra-metis__list_documents` to get all documents
...
```yaml
---
name: ultra-metis-session-start
event: SessionStart
description: Load ultra-metis project context on session start
---
```
...
**Alternative 1: Shell script hooks that call ultra-metis CLI**
...
Created 3 files in `plugins/ultra-metis/hooks/`:
- `hooks.json` — declares SessionStart and PreCompact hooks
- `session-start-hook.sh` — detects .metis, runs `ultra-metis status`, outputs full project context including: planning hierarchy, active work summary, MCP tool reference, document types/phases table, story types, document creation rules, human-in-the-loop rules, task workflow
...
- Check for `ultra-metis` CLI, warn if not installed
- Run `ultra-metis status` to get live project state
- Reference `mcp__ultra-metis__*` tools throughout
- Use ultra-metis hierarchy (ProductDoc → Epic → Story → Task)
- Include story types and document phase models

## Exit Criteria
- [x] SessionStart hook created and functional
- [x] Hook detects ultra-metis projects and surfaces active work
